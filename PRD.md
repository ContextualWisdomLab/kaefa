...
kaefa is an R package that automates exploratory factor analysis (EFA) for
complex, cross-classified multilevel data. It targets applied researchers who
need a repeatable, scalable workflow to explore factor structures, compare
models, and refine item sets with minimal manual tuning.

## Problem Statement

Exploratory factor analysis for multilevel, cross-classified data is
time-consuming and error-prone. Researchers often need to:
...
kaefa addresses these needs by providing an automated EFA framework with
parallel execution and a Shiny interface for non-programming users.
...
- Users can run an automated EFA workflow end-to-end with a single function
  call.
...
- Applied researchers in psychology or education who need automated factor
  discovery.
...
- Allow configuration of extraction counts, rotation, and model selection
  criteria.
...
- Relies on `mirt`, `psych`, `future`, `shiny`, `fitdistrplus` and related R
  packages.
